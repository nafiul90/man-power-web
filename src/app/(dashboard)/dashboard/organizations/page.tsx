      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div>
          ...
          <input
            ...
            placeholder="e.g. Green Fields People"
          />
          {errors.title && (
        ...
        <div>
          ...
          {availableOwners.length === 0 ? (
            <p className="text-[var(--muted)] text-sm">
              No Org Owner users found. Create users with Org Owner role first.
            </p>
          ) : (
            <div className="space-y-2 max-h-48 overflow-y-auto rounded-lg border border-[var(--card-border)] p-2">
              {availableOwners.map((owner) => (
                <label
                  key={owner._id}
                  className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-[var(--accent)] cursor-pointer transition-colors"
                >
                  <input
                    type="checkbox"
        ...
        <div className="flex justify-end gap-3 pt-2">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-lg border border-[var(--card-border)] text-sm hover:bg-[var(--accent)] transition-colors"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={submitting}
            className="px-6 py-2 bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white rounded-lg text-sm font-semibold disabled:opacity-60 transition-all"
          >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--card-border)] bg-[var(--accent)]/50">
                ...
                <th className="text-left px-4 py-3 font-semibold text-[var(--foreground)] hidden md:table-cell">
                  Owners
                </th>
            ...
            <tbody>
              {loading ? (
                ...
                orgs.map((org) => (
                  <tr
                    key={org._id}
                    className="border-b border-[var(--card-border)] hover:bg-[var(--accent)]/30 transition-colors"
                  >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-[var(--muted)]">
                            {org.owners.length} owner
                            {org.owners.length !== 1 ? "s" : ""}
                          </p>
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {org.owners.length === 0 ? (
                          ...
                          org.owners.map((o) => (
                            <span
                              key={o._id}
                              className="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-[var(--accent)] text-[var(--primary)] font-medium"
                            >
                              {o.fullName}
                            </span>
                          ))
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--card-border)]">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={page === 1}
                className="p-2 rounded-lg border border-[var(--card-border)] hover:bg-[var(--accent)] disabled:opacity-40 transition-colors"
              >
